...
server <- function(input, output, session) {
  ...
  output$expensePlot <- renderPlot({
    ggplot(monthly_expenses(), aes(x = Month)) +
      geom_bar(aes(y = Groceries), fill = "steelblue", stat = "identity") +
      geom_bar(aes(y = Transportation), fill = "lightblue", stat = "identity") +
      geom_bar(aes(y = Rent), fill = "lightgreen", stat = "identity") +
      geom_bar(aes(y = Entertainment), fill = "pink", stat = "identity") +
